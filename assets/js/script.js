...
$('#jump').on('click', function(e) {
    ...
    var qno = Math.trunc($('#qno').val());
    //zero indexed array. qno is again decremented in jumpTo(). Consider fixing
    if ((qno-1) <= 0 || (qno-1) > qnos.length) {
      $('#qno-label').attr('data-error', 'Question Number is between 1 and '+qnos.length);
      $('#qno').addClass('invalid');
    }
    //check if question is already answered, by seeing if selction array has no between 0 and 3
    else if (answered(qno-1)) {
      $('#qno-label').attr('data-error', 'You already answered that question');
      $('#qno').addClass('invalid');
    } else {
      $('#qno').removeClass('invalid');
      jumpTo(qno);
    }
});
...
function init() {
    ...
    oReq.open("get", "assets/json/tall.json", true);
    oReq.send();
...
// Checks if question has already been answered. Can be improved
function answered(qno)
{
  if(selections[qno]>0 && selections[qno]<=3)
  {
    return 1;
  } else {
    return 0;
...
function jumpTo(qno) {
    ...
    quiz.fadeOut(function() {
            ...
            var answerParams = checkAnswer();
            questionCounter = qno;
            var nextQuestion = createQuestionElement(qnos[qno], false);
            quiz.append(nextQuestion).fadeIn();
            if (!(isNaN(selections[qno]))) {
                $('input[value=' + selections[qno] + ']').prop('checked', true);
            }
            $('#jumpto').show();
            $('#next').hide();
            $('#submit').show();
